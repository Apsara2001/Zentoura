        ...
        <div className="min-h-screen bg-gray-50 pt-28 pb-20 px-4 md:px-8">
            <div className="max-w-5xl mx-auto">
                <div className="flex justify-between items-center mb-10">
                    <h1 className="text-3xl md:text-4xl font-display font-bold text-zentoura-deepest">{t('bookings.myJourneys')}</h1>
                ...
                {bookings.length === 0 ? (
                    <div className="bg-white rounded-3xl p-16 text-center shadow-sm">
                        ...
                        <h2 className="text-2xl font-bold text-gray-900 mb-2">{t('bookings.noBookingsYet')}</h2>
                        <p className="text-gray-500 mb-8 max-w-md mx-auto">{t('bookings.noBookingsDescription')}</p>
                        <Link
                            to="/hotels"
                                    ...
                                    <div className="flex justify-between items-start">
                                        ...
                                        <span className={`px-4 py-1.5 rounded-full text-xs font-bold uppercase tracking-wide
                                            ...
                                            {booking.status === 'Confirmed' ? t('bookings.confirmed') :
                                                booking.status === 'Cancelled' ? t('bookings.cancelled') : t('bookings.pending')}
                                        </span>
                                    </div>

                                    <div className="grid grid-cols-2 md:grid-cols-3 gap-4 py-4 border-t border-b border-gray-50">
                                        <div>
                                            <span className="block text-xs text-gray-400 uppercase font-bold mb-1">{t('bookings.checkIn')}</span>
                                            <span className="font-medium text-gray-900">{booking.checkIn}</span>
                                        </div>
                                        <div>
                                            <span className="block text-xs text-gray-400 uppercase font-bold mb-1">{t('bookings.checkOut')}</span>
                                            <span className="font-medium text-gray-900">{booking.checkOut}</span>
                                        </div>
                                        <div>
                                            <span className="block text-xs text-gray-400 uppercase font-bold mb-1">{t('bookings.roomType')}</span>
                                            <span className="font-medium text-gray-900">{booking.room?.name || 'Standard Room'}</span>
                                        </div>
                                        <div>
                                            <span className="block text-xs text-gray-400 uppercase font-bold mb-1">{t('bookings.rooms')}</span>
                                            <span className="font-medium text-gray-900">{booking.numRooms} {booking.numRooms > 1 ? t('bookings.rooms') : t('bookings.room')}</span>
                                        </div>
                                    </div>

                                    <div className="flex justify-between items-center pt-2">
                                        <div className="text-sm font-medium text-gray-500">
                                            {t('bookings.totalPrice')}: <span className="text-lg font-bold text-zentoura-primary ml-2">Rs. {booking.totalPrice}</span>
                                        </div>
